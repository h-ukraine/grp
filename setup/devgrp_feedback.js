...
function fillgrp_params_feedback(obj) {
    ...
    if (vismode == 3)
        ...
        tbl.rows.forEach(row => {
            // row.style.gridTemplateColumns = '0.4fr 0.4fr 0.4fr 0.4fr repeat(' + (tbl.colCount - 2).toString() + ', 1.5fr)';
            row.style.gridTemplateColumns = 'repeat(4, 0.5fr)  repeat(' + (tbl.colCount - 4).toString() + ', 1.9fr)';
            row.children[0].style.minWidth = '65px';
            row.children[1].style.minWidth = '65px';
            row.children[2].style.minWidth = '65px';
            row.children[3].style.minWidth = '65px';
            // row.children[0].innerText = '0';

            // row.children[1].style.minWidth = '100px';
            // row.children[2].style.minWidth = '100px';
            for (i = 4; i < tbl.colCount; i++) {
                row.children[i].style.minWidth = '90px';
            }

            if (tbl.rows.indexOf(row) == 0) {
                row.children[0].innerText = 'в даних';
                row.children[1].innerText = 'Прилад';
                row.children[2].innerText = 'Адмін.';
                row.children[3].innerText = 'Корист.';
                row.children[4].innerText = 'Дані';
                row.children[5].innerText = 'Поріг1';
                row.children[6].innerText = 'Поріг2';
                ...
                row.children[3].innerText = String.fromCodePoint(10003); //галочка
            ...
            por1.forEach(elem => {
                if (k < rc) {
                    if (vismode == 4) {
                        ...
                        tbl.row(k).style.gridTemplateColumns = 'repeat(4, 0.5fr)  repeat(' + (tbl.colCount - 4).toString() + ', 1.9fr)';
                    ...
                    if ((ainp & (1 << i)) || (vismode == 4)) {
                        ...
                        if (vismode == 4) {
                            let mesrd = 0;
                            if (lastmeasured && (lastmeasured.length > 0))
                                mesrd = lastmeasured[0].inp_mask;

                            fillcheckbox(tbl.cell(k, 0), mesrd & (1 << i));
                            fillcheckbox(tbl.cell(k, 1), ainp & (1 << i));


                            fillcheckbox(tbl.cell(k, 2), admin_inp & (1 << i));
                            fillcheckbox(tbl.cell(k, 3), user_inp & (1 << i));
            ...
            arr.forEach(j => {
                ...
                if (i < tbl.rowCount) {
                    ...
                    if ((dev[0].bitmask & 0xe23e & (1 << j)) && (vismode == 3)) {
                        ...
                        tbl.cell(i, col - 1).style.display = 'none';
                        tbl.row(i).style.gridTemplateColumns = '0.5fr 0.5fr 0.5fr 0.5fr  1.9fr 3.8fr';
                        ...
                        if (vismode == 4) {
                            let mesrd = 0;
                            if (lastmeasured && (lastmeasured.length > 0))
                                mesrd = lastmeasured[0].bit_mask;

                            fillcheckbox(tbl.cell(i, 0), mesrd & (1 << j));
                            fillcheckbox(tbl.cell(i, 1), dinp & (1 << j));

                            fillcheckbox(tbl.cell(i, 2), admin_inp & (1 << j));
                            fillcheckbox(tbl.cell(i, 3), admin_bit & (1 << j));
...
window.onload = (() => {
    ...
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    // get user list from db, then fill select 
    select = document.getElementById('user_name');
