    ...
    if (tbl && needrefresh) {
        ...
        vmass.forEach(item => {
            ...
            if ((i < tbl.rowCount) && (col < tbl.colCount)) {
                ...
                var tim = item.server_datetime.slice(11, 16); //    (11, 20);
                ...
                for (; i <= 12; i++) {
                    ...
                    if (amask) {
                        ...
                        tbl.cell(i, col).innerText = (aval < 10) && (aval > 0) ? value.toFixed(2) : value.toFixed(0);
...
var textnames = ['Дата та час', //полные названия  
    'Aкумулятор, В',
    'P1, кгс/см2',
    'P3, мм.вод.ст.',
    'Загазованість, % об.',
...
function init_tblfirst_tbleft(firsttime = false) {
    ...
    if (lastsavedDevicesObject) {
        ...
        if (dev) {
            ...
            rowamount = 1 + getbitCount(dev.inpmask & 0x07ff) + getbitCount(dev.bitmask & 0xe63f);
