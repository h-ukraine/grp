...
function fillgrp_params(obj) {
    ...
    if (vismode == 3)
        ...
        tbl.rows.forEach(row => {
            row.style.gridTemplateColumns = '0.4fr repeat(' + (tbl.colCount - 1).toString() + ', 1fr)';
            row.children[0].style.minWidth = '70px';
            ...
            if (tbl.rows.indexOf(row) == 0) {
                row.children[0].innerText = 'Актив.';
                row.children[1].innerText = 'Поріг1';
                row.children[2].innerText = 'Поріг2';

                row.children[0].style.color = color;
                row.children[1].style.color = color;
                row.children[2].style.color = color;

                row.children[0].style.background = backcolor;
                row.children[1].style.background = backcolor;
                row.children[2].style.background = backcolor;
            }
            else if (tbl.rows.indexOf(row) == 1) {
                ...
                row.children[0].style.background = gradient;
                row.children[1].style.background = gradient;
                row.children[2].style.background = gradient;


                row.children[0].style.color = 'black';
                row.children[1].style.color = 'black';
                row.children[2].style.color = 'black';
                if (tbl.rowCount > 2) {
    ...
    if (parlist.length > 0) {
        ...
        if (dev.length > 0) {
            ...
            por1.forEach(elem => {
                if (k < rc) {
                    if (vismode == 4) {
                        tbl.row(k).style.gridTemplateColumns = '0.4fr 1fr 1fr';
            ...
            arr.forEach(j => {
                ...
                if (i < tbl.rowCount) {
                    ...
                    if ((dev[0].bitmask & 0xe63f & (1 << j)) && (vismode == 3)) {
                        ...
                        tbl.cell(i, col - 1).style.display = 'none';
                        tbl.row(i).style.gridTemplateColumns = '0.2fr 1fr';
...
function init_tblfirst_tbleft(firsttime = false) {
    ...
    if (lastsavedDevicesObject) {
        ...
        if (dev) {
            ...
            var dbits = getbitCount(dev.bitmask & 0xe63f);
            rowamount = 1 + 1 + getbitCount(dev.inpmask & 0x07ff) + getbitCount(dev.bitmask & 0xe63f);
            // var fff = dev.bitmask << 16;

            mask32 = ((dev.bitmask << 16) & 0xe63f0000) | (dev.inpmask & 0x07ff);
            if (vismode == 4) {
                rowamount = 1 + 1 + getbitCount(0x07ff) + getbitCount(0xe63f);
                mask32 = 0xe63f0000 | (0x07ff);
            }
